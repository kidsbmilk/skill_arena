...
object MySQLUtils {
    ...
    // 网上那个作者写的代码，很有可能是人家使用的是配置文件，而我使用的是传参数的方法，所以人家的能运行，而我这里运行不了。
    // 等以后提交自己的任务时，都把这几个类打包到自己的包里，或者可以做成公司通用的包，再写入配置文件，这样运行就没问题了。
    ...
    def getDFFromMysql(sqlContext: SQLContext, mysqlTableName: String, queryCondition: String): DataFrame = {
        ...
        prop.put("user", DataSourceConfig.username)
        prop.put("password", DataSourceConfig.password)

        if (null == queryCondition || "" == queryCondition)
            sqlContext.read.jdbc(DataSourceConfig.url, mysqlTableName, prop)
        else
            sqlContext.read.jdbc(DataSourceConfig.url, mysqlTableName, prop).where(queryCondition)
    }
    ...
    def dropMysqlTable(sqlContext: SQLContext, mysqlTableName: String): Boolean = {
        ...
        try {
            ...
            true // 很奇怪，如果没有这个的话，执行时，在jupyter notebook里返回的结果是false
        } catch {
            case e: Exception =>
